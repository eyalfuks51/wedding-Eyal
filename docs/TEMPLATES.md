...
| `punk-poster` | `PunkPosterTemplate` | none (CSS-only decor) | "Punk indie-rock gig poster" — strict ink/hot-pink/warm-white, screen-print grain overlay, couple names as the chevron-flanked stacked "headliner" (Danidin, per-line wobble + sticker shadow), 4-point sparkle stars, setlist schedule, ticket-stub RSVP, pink-drenched directions. Smart-Adapted from a DIY gig poster. BEM namespace `.pk` |
