...
3. **Codex clones the closest existing template** as a starting point (usually `ElegantTemplate` for minimal layouts, `WeddingDefaultTemplate` for decorative ones)
4. **Codex rewrites the SCSS** for the new palette / fonts / layout, replacing all hardcoded asset references with the new files
5. **Codex registers** `'boho': BohoTemplate` in `EventPage.jsx` and updates this file
